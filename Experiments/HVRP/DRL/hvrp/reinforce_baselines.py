    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is None:
            self.dataset = self.problem.make_dataset(
                size=self.opts.graph_size, instances=self.opts.instances, num_veh=self.opts.num_veh, num_samples=self.opts.val_size, distribution=self.opts.data_distribution)
        else:
