...
def get_best(sequences, cost, veh_lists, ids=None, batch_size=None):
    ...
    result = np.full(len(group_lengths)
                     if batch_size is None else batch_size, -1, dtype=int)
...
def eval_dataset_mp(args):
    ...
    dataset = model.problem.make_dataset(
        filename=dataset_path, num_samples=val_size, offset=opts.offset + val_size * i, instances = opts.instances)
...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    if opts.multiprocessing:
        ...
        dataset = model.problem.make_dataset(
            filename=dataset_path, num_samples=opts.val_size, offset=opts.offset, instances = opts.instances)

        results = _eval_dataset(model, dataset, width,
    ...
    costs, tours, veh_lists, durations = zip(*results)
    
    print("costs:", costs[0])
    # import pdb; pdb.set_trace()
    final_cost = []
    for i in range(len(costs) // 8):
        cost_i = []
        for j in range(8):
            cost_i.append(costs[i + len(costs) // 8 * j])
        final_cost.append(min(cost_i))

    # print("Average cost: {} +- {}".format(np.mean(final_cost),
    #       2 * np.std(final_cost) / np.sqrt(len(final_cost))))
    ...
    model_name = "_".join(os.path.normpath(
        os.path.splitext(opts.model)[0]).split(os.sep)[-2:])
    if opts.o is None:
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        with torch.no_grad():
            if opts.decode_strategy in ('sample', 'greedy'):
                ...
                sequences, costs, veh_lists = model.sample_many(
                    batch, batch_rep=batch_rep, iter_rep=iter_rep)

                batch_size = len(costs)
                ids = torch.arange(
                    batch_size, dtype=torch.int64, device=costs.device)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_calc_batch_size', type=int,
                        default=10000000, help='Size for subbatches')
