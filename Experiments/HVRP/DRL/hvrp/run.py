...
def run(opts):
    ...
    if not opts.no_tensorboard:
        tb_logger = TbLogger(os.path.join(opts.log_dir, "{}_{}".format(
            opts.problem, opts.graph_size), opts.run_name))
    ...
    model_.load_state_dict(
        {**model_.state_dict(), **load_data.get('model', {})})
    if opts.eval_only:
        # Start the actual training loop
        val_dataset = problem.make_dataset(
            size=opts.graph_size, instances=opts.instances,num_veh=opts.num_veh, num_samples=opts.val_size, filename=opts.val_dataset, distribution=opts.data_distribution)
