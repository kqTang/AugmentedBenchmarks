...
def load_model(path, obj, epoch=None):
    ...
    model = model_class(
        ...
        instances=args.get('instances', None),
        num_veh=args.get('num_veh', None),
...
def sample_many(inner_func, get_cost_func, input, batch_rep=1, iter_rep=1):
    ...
    for i in range(iter_rep):
        _log_p, _log_p_veh, pi, veh_list, tours = inner_func(input, )
        #print('pi', pi.size())
        # pdb.set_trace() #看看input[0]是啥，sample_many函数中为什么输入到get_cost中的是input[0]而不是input。input又是啥
        cost, mask = get_cost_func(input, pi, veh_list, tours)
