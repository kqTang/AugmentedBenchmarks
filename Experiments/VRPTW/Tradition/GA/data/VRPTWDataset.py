...
def read(filename, capacity, sub_num_samples, size):
    ...
    solomon = np.genfromtxt(os.path.dirname(__file__)+filename, dtype=[
    ...
    high = (leave_time[0] - (loc - depot.unsqueeze(-2)
                             ).norm(p=2, dim=-1) - service_duration[1] - 1e-4).floor()
    # if filename == "./SolomonBenchmark/C1/Solomon1.txt":
    #     solomon = np.genfromtxt(
    #         os.path.dirname(__file__)+"/SolomonBenchmark/C1/Solomon1.txt", dtype=[
    #             np.float32, np.float32, np.float32, np.float32, np.float32, np.float32, np.float32])
    #     center = [((x[4]+x[5])/2 ).astype(np.float32) for x in solomon][1:]
    #     center = torch.from_numpy(np.array(center))[
    #         None, :].expand(sub_num_samples, -1)
    # elif filename == "./SolomonBenchmark/C2/Solomon1.txt":
    #     solomon = np.genfromtxt(
    #         os.path.dirname(__file__)+"/SolomonBenchmark/C2/Solomon1.txt", dtype=[
    #             np.float32, np.float32, np.float32, np.float32, np.float32, np.float32, np.float32])
    ...
    if filename == "/SolomonBenchmark/C1/Solomon1.txt":
        solomon = np.genfromtxt(
            os.path.dirname(__file__)+"/SolomonBenchmark/C1/center_time.txt", dtype=[np.float32])
        print("/SolomonBenchmark/C1/center_time.txt")
        center = [x[0] for x in solomon]
        center = torch.from_numpy(np.array(center))[
            None, :].expand(sub_num_samples, -1)
    elif filename == "/SolomonBenchmark/C2/Solomon1.txt":
        solomon = np.genfromtxt(
            os.path.dirname(__file__)+"/SolomonBenchmark/C2/center_time.txt", dtype=[np.float32])
