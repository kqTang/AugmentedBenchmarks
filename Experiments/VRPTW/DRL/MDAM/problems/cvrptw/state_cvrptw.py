...
class StateCVRPTW(NamedTuple):
    ...
    enter_time: torch.Tensor
    service_duration: torch.Tensor
    ...
    def get_final_cost(self):
        ...
        return (self.lengths + (self.coords[self.ids, 0, :] - self.cur_coord).norm(p=2, dim=-1)) * self.horizon
    ...
    def update(self, selected):
        ...
        # tw 1024,1
        arrive_time = torch.max(  
            (self.cur_time + (cur_coord - self.cur_coord).norm(p=2, dim=-1)),
            self.enter_time[self.ids, selected]
        )
        # self.service_duration 1024,100
        service_duration = torch.cat([torch.zeros_like(self.service_duration[:, 0:1]),
                   self.service_duration], dim=1)[self.ids, selected]
        cur_time = torch.where(
            selected == 0,
            torch.zeros_like(self.cur_time),
            arrive_time + service_duration            
        )
    ...
    def get_mask(self):
        ...
        # self.cur_coord 1024,1,2
        # exceeds_cap 1024,1,100
        # self.coords 1024,101,2

        # 1024,100
        route_duration_all = (self.cur_coord.clone().expand(-1, self.coords.size(-2) - 1, -1) -
                              self.coords[:,1:,:].clone()).norm(p=2, dim=-1)
        # self.leave_time 1024, 101
        tw_constraint = ((self.cur_time.expand_as(route_duration_all) + route_duration_all.clone()) >
                         self.leave_time[:, 1:])

        # Nodes that cannot be visited are already visited or too much demand to be served now
        mask_loc = visited_loc.to(
            exceeds_cap.dtype) | exceeds_cap | tw_constraint.unsqueeze(-2)
