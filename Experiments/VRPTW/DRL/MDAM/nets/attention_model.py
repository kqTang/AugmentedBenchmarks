...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp or self.is_cvrptw:
        ...
        self.project_node_embeddings = [nn.Linear(
            embedding_dim, 3 * embedding_dim, bias=False) for i in range(self.n_paths)]
        self.project_fixed_context = [nn.Linear(
            embedding_dim, embedding_dim, bias=False) for i in range(self.n_paths)]
        self.project_step_context = [nn.Linear(
            step_context_dim, embedding_dim, bias=False) for i in range(self.n_paths)]
    ...
    def forward(self, input, opts=None, baseline=None, bl_val=None, n_EG=None, return_pi=False, return_kl=False):
        ...
        for i in range(self.n_paths):
            ...
            if self.is_vrp:
                # ignore the depot for the first step
                outputs.append(log_p[:, 0, 1:])
            else:
                outputs.append(log_p[:, 0, :])
            outputs[-1] = torch.max(outputs[-1], torch.ones(outputs[-1].shape,
                                    dtype=outputs[-1].dtype, device=outputs[-1].device) * (-1e9))  # for the kl loss
            ...
            for _i in range(self.n_paths):
                for _j in range(self.n_paths):
                    ...
                    kl_divergence = torch.sum(
                        torch.exp(outputs[_i]) * (outputs[_i] - outputs[_j]), -1)
                    kl_divergences.append(kl_divergence)
            loss_kl_divergence = -opts.kl_loss * \
                torch.stack(kl_divergences, 0).mean()
        ...
        for i in range(self.n_paths):
            ...
            while not (self.shrink_size is None and states[i].all_finished()):
                if j > 1 and j % n_EG == 0:
                    if not self.is_vrp:
                        ...
                        mask_attn = mask
                    embeddings, init_context = self.embedder.change(
                        attn, V, h_old, mask_attn, self.is_tsp)
                    fixed = self._precompute(embeddings, path_index=i)
                log_p, mask = self._get_log_p(fixed, states[i], i)
                if j == 0:
                    mask_first = mask
            ...
            if baseline != None:
                if i == 0:
                    ...
                    bl_val, _ = baseline.eval(
                        input, costs[0]) if bl_val is None else (bl_val, 0)
    ...
    def _init_embed(self, input):
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            embedded_depot = self.init_embed_depot(depot_xy_tw)
            # shape: (batch, 1, embedding)
            embedded_node = self.init_embed(node_xy_demand_tw)
            return torch.cat((embedded_depot, embedded_node), dim=1)
        # TSP
    ...
    def _precompute(self, embeddings, num_steps=1, path_index=None):
        ...
        fixed_context = self.project_fixed_context[path_index](graph_embed)[
            :, None, :]

        # The projection of the node embeddings for the attention is calculated once up front
        glimpse_key_fixed, glimpse_val_fixed, logit_key_fixed = \
            self.project_node_embeddings[path_index](
    ...
    def _get_log_p(self, fixed, state, path_index, normalize=True):
        ...
        query = fixed.context_node_projected + \
            self.project_step_context[path_index](
        ...
        log_p, glimpse = self._one_to_many_logits(
            query, glimpse_K, glimpse_V, logit_K, mask, path_index)
            ...
            if num_steps == 1:  # We need to special case if we have only 1 step, may be the first or not
                if state.i.item() == 0:
                    ...
                    return embeddings.gather(
                        1,
                        torch.cat((state.first_a, current_node), 1)[
                            :, :, None].expand(batch_size, 2, embeddings.size(-1))
                    ).view(batch_size, 1, -1)
    ...
    def _get_attention_node_data(self, fixed, state):
        ...
        if self.is_vrp and self.allow_partial:
            ...
            glimpse_key_step, glimpse_val_step, logit_key_step = \
                self.project_node_step(
                    state.demands_with_depot[:, :, :, None].clone()).chunk(3, dim=-1)
