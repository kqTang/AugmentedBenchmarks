...
def plot_vehicle_routes(data, route, ax1, markersize=5, visualize_demands=False, demand_scale=1, round_demand=False):
    ...
    routes = [r[r != 0] for r in np.split(
        route, np.where(route == 0)[0]) if (r != 0).any()]
    # pdb.set_trace()
    depot = data['depot'].cpu().numpy()*data['horizon'].item()
    locs = data['loc'].cpu().numpy()*data['horizon'].item()
    ...
    for veh_number, r in enumerate(routes):
        ...
        if not visualize_demands:
            ax1.plot(xs, ys, 'o', label = 'hdjddjdjdjdjdjjjjdjdjd', mfc=color, markersize=markersize, markeredgewidth=0.0)
...
if __name__ == "__main__":
    ...
    for i in range(10):
        dataset = CVRP.make_dataset(size=50, num_samples=512)
