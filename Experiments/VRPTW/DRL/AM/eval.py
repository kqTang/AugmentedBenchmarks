...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    if (model.problem.NAME == "cvrptw") & (len(costs) <= 12):
        write(costs, tours)
    print("Average cost: {} +- {}".format(np.mean(costs),
          2 * np.std(costs) / np.sqrt(len(costs))))
    ...
    for i, (data, tour) in enumerate(zip(dataset, tours)):
        ...
        demand_scale = 1000
        plot_vehicle_routes(
            data, tour, ax, visualize_demands=False, demand_scale=demand_scale, round_demand=True)
        fig.savefig(os.path.join('images', 'RC20{}.png'.format(i+1)))
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        for seq, cost in zip(sequences, costs):
            if model.problem.NAME == "tsp":
                ...
                seq = [0] + np.trim_zeros(seq).tolist() + [0]  # Add depot
            elif model.problem.NAME in ("op", "pctsp"):
                # We have the convention to exclude the depot
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_calc_batch_size', type=int,
                        default=100000000, help='Size for subbatches')
