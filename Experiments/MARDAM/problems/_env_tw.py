...
class VRPTW_Environment(VRP_Environment):
    ...
    def __init__(self, data, nodes = None, cust_mask = None,
        ...
        if hasattr(data, 'loc_scl'):
            self.loc_scl = data.loc_scl

    def _sample_speed(self):
        return self.veh_speed
    def _update_vehicles(self, dest):
        '''
        dest is cur node feature
        self.cur_veh.shape = (batch_size, 1, 4), 
        '''
    ...
    def step(self, cust_idx):
        ...
        if self.done:
            ...
            # If there are customers that not served, the reward is added with penalty
