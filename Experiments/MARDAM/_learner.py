...
class AttentionLearner(nn.Module):
    def __init__(self, cust_feat_size, veh_state_size, model_size = 128,
            layer_count = 3, head_count = 8, ff_size = 512, tanh_xplor = 10, input_type='origin', greedy = False):
        r"""
        ...
        if input_type == 'bool':
            self.depot_embedding = nn.Linear(cust_feat_size+1, model_size)
    ...
    def _encode_customers(self, customers, mask = None):
        ...
        customers_clone = customers.clone()
       
        if self.input_type == 'origin':
            pass
        # # center and half-width
        elif self.input_type == 'center':
            center = customers_clone[:, :, 3:5].sum(dim = 2)/2
            half_width = (customers_clone[:, :, 4] - customers_clone[:, :, 3])/2
            # pdb.set_trace()
            customers_clone[:, :, 3] = center
            customers_clone[:, :, 4] = half_width
        # # differ2b0
        elif self.input_type == 'differ2b0':        
            enter2b0 = customers_clone[:, 0:1, 4] - customers_clone[:, :, 3]
            customers_clone[:,1:,4] = customers_clone[:, 0:1, 4] - customers_clone[:, 1:, 4]
            customers_clone[:, :, 3] = enter2b0
            # pdb.set_trace()
        # # # bool
        # pdb.set_trace()
        elif self.input_type == 'bool':
            bool = (customers_clone[:, :, 3:4] != 0)
            customers_clone = torch.cat((customers_clone, bool), dim=-1)

        cust_emb = torch.cat((
            self.depot_embedding(customers_clone[:, 0:1, :]),
            self.cust_embedding(customers_clone[:, 1:, :])
