...
def test_epoch(args, test_env, learner, ref_costs):
    ...
    print(
        "Cost on test dataset: {:5.2f} +- {:5.2f} ({:.2%})".format(mean, std, gap))
...
def main(args):
    ...
    if not args.use_tool_to_test_solomon:
        if args.problem_type != "vrp":
            gen_params.extend([args.horizon, args.dur_range,
                                args.tw_ratio, args.tw_range])
        ...
        if not args.use_solomon_train:
            train_data = Dataset.generate(
                args.iter_count * args.batch_size,
                *gen_params
            )
        else:
            train_data = Dataset.generate_solomon_train(
            args.train_type,
            args.iter_count * args.batch_size,
            *gen_params
        )
    ...
    if not args.use_solomon_train:
        if args.use_tool_to_test_solomon:
        ...
        test_data = Dataset.generate_solomon_train(
            args.train_type,
            args.test_batch_size,
            *gen_params
        )
    ...
    if args.problem_type != "vrp":
        ...
        if args.problem_type != "vrptw":
            env_params.extend([args.speed_var, args.late_prob,
                              args.slow_down, args.late_var])
    test_env = Environment(test_data, None, None, *env_params)

    if ref_routes is not None:
        ref_costs = eval_apriori_routes(
            test_env, ref_routes, 100 if args.problem_type[0] == 's' else 1)
        if args.use_tool_to_test_solomon:
            print("Reference cost on test dataset {:5.2f} +- {:5.2f}".format(
                ref_costs.mean(), ref_costs.std()))
            if len(ref_costs) == 56:
                for i in range(len(ref_costs)):
                    print(i,":", ref_costs[i].items())
            # pdb.set_trace()
            sys.exit()
        print("Reference cost on test dataset {:5.2f} +- {:5.2f}".format(
            ref_costs.mean(), ref_costs.std()))
    ...
    if args.baseline_type == "none":
        ...
        baseline = RolloutBaseline(
            learner, args.rollout_count, args.rollout_threshold)
    elif args.baseline_type == "critic":
        baseline = CriticBaseline(
            learner, args.customers_count, args.critic_use_qval, args.loss_use_cumul)
    baseline.to(dev)
    verbose_print("Done.")
    ...
    if args.baseline_type == "critic":
        ...
        if args.rate_decay is not None:
            lr_sched = LambdaLR(
                optim, lambda ep: args.learning_rate * args.rate_decay**ep)
    verbose_print("Done.")
    ...
    try:
        for ep in range(start_ep, args.epoch_count):
            train_stats.append(train_epoch(
                args, train_data, Environment, env_params, baseline, optim, dev, ep))
            if ref_routes is not None:
                test_stats.append(test_epoch(
                    args, test_env, learner, ref_costs))
